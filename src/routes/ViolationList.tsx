...
const ViolationList = (props: any) => {
    ...
    useEffect(() => {
        ...
        adhocTaskEstablishmentDraft.getSelectedItem()!="" ) {
            ...
            if(tempArray[0].ListOfOpportunity){
                let temp:any = tempArray[0].ListOfOpportunity
                let tempListOfOpportunityt:any = temp.TradeViolation
              
                setTaskList(tempListOfOpportunityt)
            }
    ...
    const renderRecentNews = (item: any, index: number) => {
        ...
        return (
            ...
            <TouchableOpacity
                onPress={() => {
                    NavigationService.navigate('ViolationDetails',{"ViolationDetails":item})
                }}
                ...
                <TableComponent
                    ...
                    data={[{ keyName: Strings[context.isArabic ? 'ar' : 'en'].serviceRequstList.violationNumber, value: item.ViolationNumber },
                    { keyName: Strings[context.isArabic ? 'ar' : 'en'].serviceRequstList.inspectionNumber, value: item.InspectionNumber },
                    { keyName: Strings[context.isArabic ? 'ar' : 'en'].serviceRequstList.status, value: item.Status },
        ...
        <SafeAreaView style={{ flex: 1 }}>
            ...
            <ImageBackground style={{ height: HEIGHT, width: WIDTH }} source={context.isArabic ? require('./../assets/images/backgroundimgReverse.jpg') : require('./../assets/images/backgroundimg.jpg')}>
                ...
                <View style={{ flex: 0.4, flexDirection: 'row', width: '80%', justifyContent: 'center', alignItems: 'center', alignSelf: 'center', borderRadius: 18, backgroundColor: '#c4ddd2' }}>
                    <Text style={{ color: fontColor.TitleColor, fontSize: 12, textAlign: context.isArabic ? 'right' : 'left', fontWeight: 'bold', fontFamily: context.isArabic ? fontFamily.arabicTextFontFamily : fontFamily.textFontFamily }}>{(Strings[context.isArabic ? 'ar' : 'en'].serviceRequstList.violationList)} </Text>
                </View>
                <View style={{ flex: 0.2 }} />
...
export default observer(ViolationList);