...
export default class Dropdown extends PureComponent {
  static defaultProps = {
    ...
    valueArabicExtractor: ({ valueArabic } = {}, index) => valueArabic,
    propsExtractor: () => null,
  ...
  static propTypes = {
    ...
    valueArabic: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.number,
    ]),
  ...
  onSelect(index) {
    ...
    let valueArabic = valueArabicExtractor(data[index], index);
  ...
  selectedIndex() {
    let { value } = this.state;
    let { valueArabic } = this.state;
    let { data, valueExtractor, valueArabicExtractor } = this.props;
  ...
  renderBase(props) {
    ...
    // alert("title" + JSON.stringify(dataa)); 
    ...
    if ('function' === typeof renderBase) {
      return renderBase({ ...props, title, value, valueArabic, renderAccessory });
    }

    // title = null == title ? itemTextStyle && itemTextStyle.textAlign == 'right' ? dataa.valueArabic :
    //   dataa.value :
    //   String(title);
    ...
    return (
      <TextInput
        ...
        {...props}
        style={[itemTextStyle ? itemTextStyle : null]}
        value={title}
  ...
  renderItem({ item, index }) {
    ...
    let valueArabic = valueArabicExtractor(item, index);
    let label = labelExtractor(item, index);

    let title = null == label ? itemTextStyle && itemTextStyle.textAlign == 'right' ? valueArabic :
      value :
      label;
